...
def insert_text(frame, text):
    ...
    ImageDraw.Draw(txtLayer, mode='RGBA').multiline_text(
        (8, 8), text, font=font, fill='white', align='center')
...
def insert_viseme_window(frame, viseme_list):
    ...
    vh = visemes.shape[0]
    visemes = add_rect(
        visemes, 0, 0, viseme_list[0].shape[1], viseme_list[0].shape[0], (255, 255, 0))
    ratio = vw/vh
    vw = fw
...
frameWindow = np.zeros(
    (config['frame_length'], config['frame_height'], config['frame_width'], 3), dtype='uint8')

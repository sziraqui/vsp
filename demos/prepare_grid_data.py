...
video_list = glob(os.path.join(dataset_path, videos_path) + '/*.mpg')[sample_start: sample_end+1]
video_list.sort()
...
X = np.zeros((sample_size, 75, out_img_height, out_img_width, 3))
Y = np.zeros((sample_size, 75, 28))
...
with h5py.File(f"../datasets/grid_lips_{sample_start}-{sample_end}_100x50.hdf5", "w") as f:
    f.create_dataset("features", data=X, dtype='i1', compression="gzip", compression_opts=4)
    f.create_dataset("labels", (sample_size,75, 28), 'i1', Y, compression='gzip', compression_opts=4)
